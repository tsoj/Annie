...
proc checkIfCommentApplicable*(commentType: TypedParam[weBlundered], cci: CommentConditionInfo): bool =
    ...
    if cci.lastEval.isNone or cci.evalDiff.isNone:
        return false

    if cci.lastEval.get notin (-1000.cp + cci.evalDiff.get)..100.cp:
        return false

    if cci.gameState.positionMoveHistory.len <= 8:
        return false

    return cci.evalDiff.get <= -blunderMargin[cci.difficultyLevel] 
